...
// ===== Component =====
export default function QuotePrototypeDemo() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="sticky top-0 z-20 bg-white border-b px-4 py-3 flex items-center gap-2 shadow-sm">
        <div className="text-lg font-semibold text-gray-800">报价配置原型演示</div>
        <div className="ml-auto flex items-center gap-2">
          ...
          <button className="px-3 py-1.5 rounded-lg bg-blue-600 text-white hover:bg-blue-700 transition-colors">
            下载明细
          </button>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 mt-4">
        <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm grid grid-cols-3 gap-4">
          <div className="flex flex-col text-sm">
            <span className="text-gray-500">税前合计</span>
            <span className="font-semibold text-blue-700 text-lg">{fmtCny(totals.total)}</span>
          </div>
          <div className="flex flex-col text-sm">
            <span className="text-gray-500">受控合计</span>
            <span className="font-medium text-gray-700">{fmtCny(totals.controlled)}</span>
          </div>
          <div className="flex flex-col text-sm">
            <span className="text-gray-700">非受控合计</span>
            <span className="font-medium text-gray-700">{fmtCny(totals.uncontrolled)}</span>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-4">
        {visibleGroups.map((g) => {
          ...
          return (
            <div key={g.id} className="mb-4 rounded-xl overflow-hidden shadow-sm border border-gray-200 bg-white">
              {/* 组头 */}
              <div className="flex items-center gap-3 px-4 py-3 bg-gray-50 border-b border-gray-200">
                <button
                  className="w-7 h-7 grid place-items-center rounded-lg border border-gray-300 hover:bg-gray-100 transition-colors"
                  onClick={() => toggleGroup(g.id)}
                >
                  {g.collapsed ? "+" : "−"}
                </button>
                <div className="font-medium text-gray-800">{g.name}</div>
                {g.version && <div className="text-gray-500 text-sm">{g.version}</div>}
                ...
                <div className="ml-auto flex items-center gap-4 text-sm">
                  ...
                  <div className="flex items-center gap-1">
                    <span className="text-gray-500">套数</span>
                    <input
                      type="number"
                      min={1}
                      className="w-20 px-2 py-1 border border-gray-300 rounded-lg text-right focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                      value={g.setCount || 1}
                  ...
                  <div className="flex items-center gap-2 text-gray-700">
                    <span className="text-gray-500">单套</span>
                  ...
                  <table className="min-w-full text-sm">
                    <thead>
                      <tr className="bg-white border-b border-gray-200">
                        <th className="text-left px-4 py-3 w-[22%] font-medium text-gray-700">物料名</th>
                        <th className="text-left px-2 py-3 font-medium text-gray-700">规格/说明</th>
                        <th className="text-center px-2 py-3 font-medium text-gray-700">单位</th>
                        <th className="text-right px-2 py-3 font-medium text-gray-700">标准价</th>
                        <th className="text-center px-2 py-3 font-medium text-gray-700">折扣%</th>
                        <th className="text-right px-2 py-3 font-medium text-gray-700">成交单价</th>
                        <th className="text-center px-2 py-3 font-medium text-gray-700">数量</th>
                        <th className="text-right px-2 py-3 font-medium text-gray-700">小计</th>
                        <th className="text-center px-2 py-3 font-medium text-gray-700">受控</th>
                      </tr>
                    </thead>
                    <tbody>
                      {g.items.map((it) => {
                        ...
                        return (
                          <tr key={it.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                            <td className="px-4 py-3 align-top">
                              ...
                              {it.spec && <div className="text-xs text-gray-500 mt-1">{it.spec}</div>}
                            </td>
                            <td className="px-2 py-3 align-top text-gray-600">{it.spec || "-"}</td>
                            <td className="px-2 py-3 text-center text-gray-700">{it.unit}</td>
                            <td className="px-2 py-3 text-right tabular-nums text-gray-700">{money(it.listPrice)}</td>
                            ...
                            <td className="px-2 py-3 text-right tabular-nums font-medium text-gray-800">{money(unitPrice)}</td>
                            <td className="px-2 py-3 text-center">
                              <input
                                type="number"
                                className="w-20 px-2 py-1 border border-gray-300 rounded-lg text-right focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                            ...
                            <td className="px-2 py-3 text-right font-semibold text-gray-900 tabular-nums">{money(sub)}</td>
                            <td className="px-2 py-3 text-center">
                              {it.controlled ? (
                                <span className="px-2 py-1 rounded-full text-xs bg-amber-100 text-amber-700 font-medium">受控</span>
                              ) : (
      ...
      <div className="sticky bottom-0 bg-white border-t border-gray-200 px-4 py-3 flex items-center justify-between shadow-lg">
        <div className="text-sm text-gray-600">
          总计：<span className="font-semibold text-blue-700 text-lg">{fmtCny(totals.total)}</span>
        </div>
